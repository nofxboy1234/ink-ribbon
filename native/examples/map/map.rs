...
// High-resolution collision mask (1 = wall / obstacle / locked door), 2px cells.
const SOLID_BIN: &[u8] = include_bytes!("../../assets/floor-1-solid.bin");
...
impl Solid {
    fn from_bytes(bytes: &'static [u8]) -> Solid {
        // header: u32 LE cell_px, u32 LE width, u32 LE height
        let w = u32::from_le_bytes([bytes[4], bytes[5], bytes[6], bytes[7]]) as i32;
        let h = u32::from_le_bytes([bytes[8], bytes[9], bytes[10], bytes[11]]) as i32;
        Solid {
    ...
    fn blocked_source(&self, sx: f32, sy: f32) -> bool {
        let x = ((sx - FLOOR1_X) / FLOOR1_W * self.w as f32).floor() as i32;
...
fn player_center_pan(l: &Layout, zoom: f32, player: (f32, f32)) -> (f32, f32) {
...
fn pan_to_center_player(state: &mut State) {
    ...
    let (x, y) = player_center_pan(&state.layout, DEFAULT_ZOOM, state.player);
...
fn update_player(state: &mut State, delta: f32) {
    ...
    while remaining.0.abs() > 1e-4 || remaining.1.abs() > 1e-4 {
        ...
        let (pos, blocked_x, blocked_y) =
            resolve_move_terrain(&state.nav, &state.solid, state.player, step);
        state.player = pos;
        if blocked_x {
...
mod tests {
    ...
    fn camera_follow_is_continuous_while_input_is_held() {
        ...
        let mut follow = player_center_pan(&l, DEFAULT_ZOOM, player);
